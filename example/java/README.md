...
java '-Djava.library.path=.' com/edts/tdlib/example/Example
```

If you built JSON interface example using `-DTD_JSON_JAVA=ON` option, then use the command `java '-Djava.library.path=.' com/edts/tdlib/example/JsonExample` instead.
